...
class CellAnalyzer:
    ...
    def analyze_neighbors(self, target_cell_id, output_dir):
        ...
        if len(level_data) == 0:
            print(f"❌ 时间点 {time_point} 的 Level {level} 数据为空")
            return
        ...
        print(f"🔍 找到 {len(target_cells)} 个 {target_cell_id} 细胞")
        ...
        for annotation in neighbor_cells['annotation'].unique():
            ...
            if cell_count > 1:
                ...
                compactness = np.mean(intra_distances) if intra_distances else 0
            ...
            print(f"    重心位置: ({centroid_x:.1f}, {centroid_y:.1f}), "
                f"{cell_count}个细胞, 距目标 {distance_to_target:.1f}, "
                f"紧密度 {compactness:.2f}")

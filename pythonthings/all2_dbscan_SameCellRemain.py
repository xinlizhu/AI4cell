...
class CellAnalyzer:
    def __init__(self, base_path="F:/allCellChat_level6_withKJ/", output_base="F:/2version/js/components/pathSelection/Every_cell_info_withKJ2"):
    ...
    def analyze_neighbors(self, target_cell_id, output_dir):
        ...
        if len(target_cells) == 0:
            print(f"❌ 未找到细胞类型 {target_cell_name} 的嵌入索引 {embedding_index}")
            return
        
        print(f"🔍 找到 {len(target_cells)} 个 {target_cell_name} 细胞（嵌入索引 {embedding_index}）")
        ...
        for annotation in neighbor_cells['annotation'].unique():
            ...
            # 如果是目标细胞类型，但嵌入索引不为目标索引，则保留
            if annotation == target_cell_name:
                annotation_cells = annotation_cells[annotation_cells['embedding_index'] != embedding_index]
